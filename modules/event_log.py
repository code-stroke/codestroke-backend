...
def get_event_log_limit(user_info):
    try:
        ...
        output = {
            ...
            "debugmsg": "Insufficient or invalid params.",
        }
        return jsonify(output)
    if start and number:
        cursor = ext.connect_()
        query = "select * from event_log order by id desc limit {},{}".format(
            start - 1, number
        )
        ...
        output = (
            {
                "success": False,
                "error_type": "parameters",
                "debugmsg": "Positive integers for params only.",
            },
            400,
        )
    return jsonify(output)
        ...
        output = (
            {
                "success": False,
                "error_type": "parameters",
                "debugmsg": "Date improperly formatted.",
            },
            400,
        )
        return jsonify(output)
