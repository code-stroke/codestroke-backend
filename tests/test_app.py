...
def client():
    ...
    test_admin = ['TEST', 'ADMIN', app.config['TEST_ADMIN_USERNAME'], app.config['TEST_ADMIN_PASSWORD'],  'admin@test.com',]
...
def client_cred(client):
    ...
    credentials = ("{}:{}".format(app.config['TEST_ADMIN_USERNAME'],
                                  app.config['TEST_ADMIN_PASSWORD']))

    headers = {
        'Authorization': ('Basic ' + b64encode(bytes(credentials, 'UTF-8')).decode('UTF-8'))
    }

    print(headers)

    post_data = {"first_name": app.config['TEST_CLIN_FIRST'],
    ...
    response = client.post("/clinicians/register_TEMP/",
                           json=post_data,
                           headers=headers
    )
